...
<form method="post" action="/myweb/createtakeout">
    <table>
        <tr><td>Restaraunt Name:</td> <td><input type="text" name="name" value="<c:out value="${selectedtakeout.name}"/>"></td></tr>
        <tr><td>Type of Cuisine:</td> <td><input type="text" name="type" value="<c:out value="${selectedtakeout.cuisineType}"/>"></td></tr>
...
<table border="1">
    ...
    <c:forEach items="${thetakeoutlist}" var="thetakeout">
        <tr><td><a href="/myweb/selecttakeout?id=<c:out value="${thetakeout.id}"/>">EDIT</a> </td><td><c:out value="${thetakeout.name}"/></td><td><c:out value="${thetakeout.cuisineType}"/></td><td><c:out value="${thetakeout.distance}"/></td><td><a href="/myweb/deletetakeout?remove=<c:out value="${thetakeout.id}"/>">Remove This Item</a> </td></tr>
    </c:forEach>
</table>
